...
export function registerGitHubShipmentCycleTool(server: McpServer, options: { readonly config: GitHubConfig | undefined; readonly transport: GitHubShipmentPocTransport | undefined; readonly authorizationSecret: string | undefined; readonly environment: NodeJS.ProcessEnv; readonly configurationError?: string }): void {
  server.registerTool("github_shipment_cycle", { title: "Run the durable GitHub shipment cycle", description: "Run the production Task/Attempt feature shipment boundary: create or reuse one feature PR, correlate exact CI, record bounded failure evidence and create one retry Attempt, or merge the feature PR only after exact green CI. This tool never creates a release PR, merges main, dispatches workflows, or deploys.", inputSchema, outputSchema, annotations: { readOnlyHint: false, idempotentHint: true, destructiveHint: true, openWorldHint: true } }, async (input, context) => {
